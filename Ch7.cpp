...
class Token_stream {
public:
	Token_stream() :full{ false }, buffer{ 0 } { }	//make an empty Token_stream
	Token get();
...
const string quitkey = "exit";	// Keywords

const string sqrtkey = "sqrt";	// functions
...
Token Token_stream::get()
{
	...
	if (full) { full = false; return buffer; }	// cheks if buffer is empty
												// if not empty returns buffer else gets the next Token
	char ch;
	cin >> ch;
	switch (ch) {
	case '(':
...
void Token_stream::ignore(char c)	// Clear input until c or '\n' is found
{
	if (full && c == buffer.kind) {// First inspect if a Token of c kind is buffered
		full = false;
		return;
	}
...
double declaration()		// cheks for declaration
{
	Token t = ts.get();
